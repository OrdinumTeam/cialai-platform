...
function UserStep({ users, choice, setChoice, newUser, setNewUser, busy, error, advance, back }) {
  return <div className="mac-network-setup__body">
    ...
    {choice === 'new' ? <div className="mac-network-setup__fields"><label className="mac-network-setup__field"><span>{translate('desktop.network.identifier')}</span><input className="field__control" value={newUser.name} placeholder={translate('desktop.network.identifierPlaceholder')} spellCheck="false" onChange={(event) => setNewUser((current) => ({ ...current, name: event.target.value }))} /></label><label className="mac-network-setup__field"><span>{translate('desktop.network.displayName')}</span><input className="field__control" value={newUser.displayName} placeholder={translate('desktop.network.displayNamePlaceholder')} onChange={(event) => setNewUser((current) => ({ ...current, displayName: event.target.value }))} /></label></div> : null}
    {error ? <p className="mac-network-setup__error" role="alert">{error}</p> : null}
