...
export default function Sidebar({ views, active, onNavigate, hidden, tunnelStatus, onOpenPair, onOpenPreferences }) {
  ...
  })}</nav><div className="mac-sidebar__footer"><button type="button" className="mac-sidebar__pair" onClick={onOpenPair}><Link2 aria-hidden="true" /><span><strong>{translate('desktop.action.pairPhone')}</strong><small><i className={`mac-dot is-${tunnelStatus?.tone || 'idle'}`} aria-hidden="true" />{tunnelStatus?.label || translate('desktop.tunnel.notConfigured')}</small></span></button><button type="button" className="mac-sidebar__settings" onClick={onOpenPreferences} title={translate('desktop.preferences.title')} aria-label={translate('desktop.preferences.title')}><Settings aria-hidden="true" /></button></div></aside>;
}
