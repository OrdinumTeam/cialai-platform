...
export default function CommandPalette({ open, onClose, views, actions, appearance }) {
  ...
  const items = useMemo(() => [
    ...
    { id: 'reload', kind: 'Ação', label: 'Recarregar navegador ativo', shortcut: '⌘R', icon: RefreshCw, run: actions.reloadData },
    { id: 'sidebar', kind: 'Ação', label: 'Mostrar ou ocultar barra lateral', shortcut: '⌃⌘S', icon: PanelLeft, run: actions.toggleSidebar },
    { id: 'prefs', kind: 'Ação', label: 'Preferências', shortcut: '⌘,', icon: Settings, run: actions.openPreferences },
    { id: 'new-terminal', kind: 'Terminais', label: 'Novo terminal', shortcut: '⌘T', icon: SquareTerminal, run: actions.newTerminal },
    { id: 'new-file', kind: 'Terminais', label: 'Novo arquivo temporário', shortcut: '⌘N', icon: FilePlus2, run: actions.newFile },
  ...
  return <div className="mac-palette-backdrop" onMouseDown={onClose}><div className="mac-palette" role="dialog" aria-label="Buscar comandos" onMouseDown={(event) => event.stopPropagation()}><div className="mac-palette__search"><Search size={17} strokeWidth={2} aria-hidden="true" /><input ref={inputRef} value={query} onChange={(event) => { setQuery(event.target.value); setIndex(0); }} onKeyDown={onKeyDown} placeholder="Buscar comandos" spellCheck={false} aria-label="Buscar" /><kbd>esc</kbd></div><ul className="mac-palette__list" role="listbox" ref={listRef}>{filtered.length === 0 && <li className="mac-palette__empty">Nada encontrado</li>}{filtered.map((item, position) => { const Icon = item.icon; const active = position === index; return <li key={item.id} role="option" aria-selected={active} className={`mac-palette__item${active ? ' is-active' : ''}`} onMouseEnter={() => setIndex(position)} onClick={() => run(item)}><Icon size={15} strokeWidth={1.75} className="mac-palette__icon" aria-hidden="true" /><span className="mac-palette__label">{item.label}</span><span className="mac-palette__kind">{item.hint || item.kind}</span>{item.shortcut && <kbd className="mac-palette__shortcut">{item.shortcut}</kbd>}</li>; })}</ul></div></div>;
}
