...
export default function PairingDialog({ open, onClose, onDevices, onConfigure }) {
  ...
  const content = !networkIsConfigured(tunnel.network) ? <div className="mac-pair__empty"><span><Link2 aria-hidden="true" /></span><h3>{translate('desktop.pair.configureFirst')}</h3><p>{translate('desktop.pair.configureDescription')}</p><button type="button" className="btn btn-primary" onClick={() => { onClose(); onConfigure?.(); }}>{translate('desktop.pair.openNetworkPreferences')}</button></div>
    : completed ? <div className="mac-pair__success"><span><Check aria-hidden="true" /></span><p className="mac-pair__eyebrow">{translate('desktop.pair.completed')}</p><h3>{translate('desktop.pair.linked', { name: completed.name })}</h3><p>{translate('desktop.pair.completedDescription')}</p><button type="button" className="btn btn-primary" onClick={() => { onClose(); onDevices?.(); }}>{translate('desktop.pair.openDevices')}</button></div>
      : <div className="mac-pair">
        <div className="mac-pair__copy"><span className="mac-pair__device"><Smartphone aria-hidden="true" /></span><p className="mac-pair__eyebrow">{tunnel.network?.desktopName}</p><h3>{translate('desktop.action.pairPhone')}</h3><p>{translate('desktop.pair.instructions')}</p><div className="mac-pair__state"><span className={`mac-dot is-${tunnel.status.tone}`} aria-hidden="true" /><span>{tunnel.status.label}</span></div></div>
        <div className="mac-pair__visual">
          <div className={`mac-pair__qr${state === 'loading' ? ' is-loading' : ''}`} style={{ '--pair-progress': `${Math.max(0, Math.min(1, clock.rotationSeconds / PAIR_ROTATION_SECONDS)) * 360}deg` }}>
            {pair?.payload ? <canvas ref={canvasRef} width="280" height="280" aria-label={translate('desktop.pair.qrLabel')} /> : <div className="mac-pair__qr-placeholder" role="status">{translate('desktop.pair.generating')}</div>}
          </div>
