...
const alpn = "cialai-directpath/1"
...
func loadOrCreateIdentity(path string) (*identity, bool, error) {
	...
	if err == nil {
		if runtime.GOOS != "windows" {
			...
			if statErr != nil {
				return nil, false, fmt.Errorf("stat identity: %w", statErr)
			}
			if info.Mode().Perm()&0o077 != 0 {
				return nil, false, fmt.Errorf("identity permissions must be 0600, got %04o", info.Mode().Perm())
			}
		}
...
func selfSignedCertificate(private ed25519.PrivateKey, public ed25519.PublicKey) (tls.Certificate, error) {
	...
	template := &x509.Certificate{
		SerialNumber: serial,
		Subject:      pkix.Name{CommonName: "cialai-directpath-" + fingerprint(public)},
