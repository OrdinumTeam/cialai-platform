...
func TestConfigFromEnvironment(t *testing.T) {
	...
	if config.Duration != 30*time.Minute || config.BurstInterval != 5*time.Second || config.ReportPath != "/tmp/cialai-soak.json" {
...
func TestConfigRejectsUnsafeIntervals(t *testing.T) {
	for _, interval := range []string{"0s", "60s", "invalid"} {
		t.Run(interval, func(t *testing.T) {
			t.Setenv("CIALAI_SOAK_BURST_INTERVAL", interval)
