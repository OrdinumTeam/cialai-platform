...
| Desktop | Matriz macOS arm64 e Intel em `macos-14`, Linux em `ubuntu-22.04` e Windows em `windows-2022`. `signing-mode.mjs` escolhe a assinatura e grava `tauri.release.conf.json`. O `tauri-action@v1` envia os instaladores com nomes sem versão, como `Cialai_aarch64.dmg`, para que `/releases/latest/download` continue valendo |
...
| --- | --- | --- |
| macOS | Developer ID e notarização com os secrets `APPLE_CERTIFICATE`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_SIGNING_IDENTITY`, `APPLE_ID`, `APPLE_PASSWORD` e `APPLE_TEAM_ID` | Assinatura ad hoc com `signingIdentity` igual a `-`, que abre no Apple Silicon depois da liberação em Privacidade e Segurança |
