...
* The application icon and every brand mark are generated, not drawn by hand. The icon is the mantis head in colour on a white rounded plate, built from two finished brand pieces that carry the artwork in the same place, so the desktop plate is the approved piece reduced and no curve is rasterized here. `tools/brand/build-app-icons.mjs` writes the three icon sources and the interface mark from the white and the black master, and `tools/brand/build-site-brand.mjs` writes the brand files of the website, each one at the exact size of the file it replaces so no page markup has to change. The gate cross reads the gradient ends and the symbol height from the generator, so the files and the code cannot drift apart
...
Preview dated 2026-09-22. The icon goes back to the colour artwork on a white rounded plate, on every system and in the browser tab, and the `cialai` command is finally found on Linux.

### Added

* A way for a delivery to ship without an announcement. A line in the release page ficha refuses the Discord message, and it holds on both paths, the automatic send from the tag and the manual resend, because the automatic one has nobody to pass an argument to

### Fixed

* The application icon is the mantis head in colour on a white rounded plate again, on the desktop, on both stores and in the browser tab. The framing differs per system and each difference has a reason: the plate fills the frame on the phone because the system rounds it, and steps back to the 824 of 1024 grid on the desktop so the icon does not touch its neighbours in the Dock. The gate reads the grid from the generator and asserts the plate is centred at exactly that size
* The `cialai` command is found on Linux. Typing it gave command not found, and the application still reported everything was fine. The cause is an ordering trap: the Debian and Ubuntu `~/.profile` only adds `~/.local/bin` to PATH when the folder already exists, and it runs at login, before Cialai creates the folder; opening a new terminal did not help either, because a terminal opens an interactive shell, which reads `~/.bashrc`
* Writing to the shell file reverses a decision from 0.2.7, because the promise behind it did not hold. The block is marked on both sides, guarded so it cannot stack the folder on PATH, backed up before the first write and removed exactly from Preferences. On fish nothing is edited: a file of its own lands in `conf.d`
* The PATH probe stopped lying. It measured a freshly opened login shell, where the folder already exists, so it answered that everything was fine while the user's terminal found nothing
