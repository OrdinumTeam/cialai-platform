...
3. `export_public.py` usa `git archive HEAD`, então só leva o que foi commitado. Em 14/09/2026 a exportação do `dd301fb` era idêntica ao `main` público `65918f8`, com diferença só numa pasta vazia; copiar por cima do clone de `Cialai/cialai`, rodar `audit_public.py` e a suíte antes do push. Desde 15/09/2026 o clone guarda documentos internos em `_INTERNAL/`, ignorados pelo Git em `.git/info/exclude` e no `.gitignore`; a cópia precisa ser `rsync -a --delete --exclude .git --exclude _INTERNAL`, porque sem esse exclude o `--delete` apaga a pasta. Nunca rodar `git clean -x` nem usar o clone como pasta de saída do `export_public.py`, que apaga a pasta de saída inteira.
...
- Retomada noturna de 14 a 15/09/2026, a partir da onda 4. O usuário pediu para avançar até terminar, testar, documentar em `Cialai/draft/relatorios`, buildar tudo e atualizar o site, com build no ORDINUM-SSD, e reforçou que quer tudo funcional já na conectividade nova. O plano passou a viver em `/Users/focoamorim/Ordinum/Repos/Cialai/draft/plan-conectividade.md` e os repositórios em `/Users/focoamorim/Ordinum/Repos`; caches e worktrees em `/Volumes/ORDINUM-SSD/Build/cialai`. Contrato comum do sidecar v2, da interface e do celular em `/Volumes/ORDINUM-SSD/Build/cialai/evidence/contrato-v2.md`. Integrados em `conectividade/produto`: `06e0931` exportação pública sem caminho fixo; CON-061 revogação nos dois transportes com 4401 antes do corte, `305d487` e `63c2a8a`; CON-041 cascata do Tor e Job Object, `a69ddfe`, parcial até o roteiro manual nos três sistemas; CON-024 rendezvous, `6912ffb`; CON-044 vigília, `587a6c0`, parcial até a sessão física de 20 min; CON-052 anúncio DNS-SD, `6e5e45c`, parcial até a descoberta nativa; CON-025 gerenciador de caminho, `3294c7d`; CON-042 e CON-043 interface v2 sem Headscale, `316d41e`, `a8f5355` e `6620fe4`; CON-029 e CON-045 sidecar RPC v2 e ponte pelo registro, `dc4a7d0` a `567e48b`, com mDNS ligado em `ac82a02`; correções de integração `04e3272` e a seguinte. Smoke do sidecar com Tor real: `net.start` em 25 ms, onion publicado em 18,9 s, `shutdown` sem `tor` órfão. Suíte completa `npm test` com node 22.23.2 aprovada depois das correções, com 198 testes Rust e 111 Jest. Em andamento: CON-028, CON-030, CON-063 e CON-064 no núcleo móvel, CON-053 a CON-055 no TypeScript do celular e CON-046 no autoteste.
- Continuação da noite de 15/09/2026, depois das ondas 4 a 6. Integrados em `conectividade/produto`: CON-053 a CON-055 no TypeScript do celular; CON-046 autoteste com rede automática e QR v2 lido do canvas; CON-080 roteiro `docs/testes/roteiro-conectividade.md`; CON-028, CON-030, CON-063 e CON-064 no núcleo Go do celular sobre o gerenciador de caminho, com ponta a ponta em processo e com Tor real; CON-032, tarefa nova, canal de controle do rendezvous pela reserva com ALPN `cialai-control/1` e furo de NAT ligado no celular; CON-050 Tor.framework e NWBrowser no iOS, com o app compilado no simulador pelo runner público; CON-051 tor-android 0.4.9.11 e NSD no Android, com APK release pareando no emulador contra o sidecar real do Mac pelo direto e pela reserva; CON-031 e CON-060 laboratório de NAT em Docker com oito cenários e proteções do pareamento nos dois transportes; documentação da prévia sem Headscale obrigatório; versão 0.2.0; correção do código de aprovação, que o computador sorteava e o celular nunca via e agora é derivado do `pairId` e da chave do celular nos dois lados; `headscale-integration.yml` só manual porque o teste antigo não compila com o sidecar v2; AppImage com o `LD_LIBRARY_PATH` do tor no bundle, porque o linuxdeploy não achava `libevent-2.1.so.7`; escrita atômica com repetição no Windows. Bindings do run público 34931518164: XCFramework zip 20.601.004 bytes e AAR 13.597.260, menos 55,8% e 54,6%. Suíte completa local, suíte Go com race e laboratório verdes no HEAD. A candidata final está no PR https://github.com/Cialai/cialai/pull/1 do público. Pendências para o usuário: aparelhos físicos e redes reais do roteiro, Codemagic, revisão da GPL do `libtor.so` no Android e Authenticode no Windows.
- Publicação de 15/09/2026. O smoke do APK 0.2.0 no emulador contra o sidecar do mesmo commit revelou dois defeitos, corrigidos antes da tag: a página do celular nunca abria a ponte, porque o proxy redirecionava para `/` sem `bridge` e a página não deduz ponte em loopback, commit `9dec080`; e a digitação rápida duplicava texto por causa do xterm 6 com o key code 229 do teclado Android, commit `2a7401c` com `packages/ui/src/terminals/ime-input.js`. No reteste, a página conectou à ponte em 437 ms, o terminal ecoou cada tecla, o código de aprovação foi igual nos dois lados, a reconexão sem QR voltou à ponte e a revogação chegou com 4401. CI do público verde nos três sistemas no run 34939601512 e `net-lab.yml` verde; `main` do público em `c216e71` com a tag `v0.2.0`; release publicada pelo run 34941492987 com 19 arquivos e o APK anexado antes da publicação; dez links `latest/download` com HTTP 200; app do macOS notarizado com o tor aninhado assinado. Codemagic `6aa8eea53abb63db7da17e11` gerou o build 2 da 0.2.0, VALID e IN_BETA_TESTING no TestFlight interno. Site `cialai.com.br` e perfil `Cialai/.github` publicados sem Headscale. `main` do privado igual a `conectividade/produto`. A prévia saiu antes do roteiro físico por pedido do usuário; CON-100 fica aberta até essa validação. Relatório em `Cialai/draft/relatorios/2026-09-15-madrugada.md`.
