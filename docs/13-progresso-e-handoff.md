...
Resultados de CI. O run `34813846975` do público, no commit `89ea40b`, ficou verde nos três sistemas: 168 casos Rust no Ubuntu 22.04 com `check:text` e os três checks de navegador por Playwright, macOS 14 e 159 casos Rust no Windows 2022, com Go, Jest e bundle sem assinatura. O run `34815427824`, no `bca04c6`, repetiu o verde. A integração com Headscale passou no push dos dois repositórios, por último no run `34812765185`. No privado, a CI roda uma vez no push final desta entrada; as rodadas intermediárias foram canceladas pelos pushes seguintes.
...
Custo. As rodadas de CI no privado usam minutos pagos de macOS e Windows; o relatório de uso com dados até 01:49 UTC de 14/09/2026 mostrava 471 minutos e custo líquido zero. Os pushes seguintes ao privado foram agrupados para uma única CI final.
