...
## 044 Comando cialai instalado pelo app, sem editar arquivo de shell

Data: 21/09/2026. Contexto: quem instala o Cialai não ganha nada no terminal. O pedido é que digitar `cialai` abra o app em qualquer máquina, para quem instalou por dmg, msi, deb, rpm ou AppImage. O `install.sh` público mora no repositório do site e cobre só macOS e Linux, então ele não serve como caminho único.

Decisão: o próprio app instala o comando, na primeira abertura, e confere o conteúdo a cada abertura. O script é embutido em tempo de compilação e gravado com o mesmo cuidado do gancho do Claude Code: escrita atômica, gravação só quando o conteúdo muda, permissão de execução no unix e cópia de segurança antes de sobrescrever. Arquivo com esse nome que não carregue o marcador `cialai-command v1` nunca é sobrescrito nem apagado sem o usuário pedir. O comando apenas abre o app, sem argumentos nem subcomandos.

No POSIX o destino é `~/.local/bin` e o app **não edita arquivo de shell nenhum**. O raio de dano não justifica o contrário: um `.zshrc` corrompido quebra todo shell novo da máquina, enquanto a falha evitada é apenas o comando não ser encontrado. Não existe um arquivo certo a escolher entre `.zshrc`, `.zprofile`, `.bashrc`, `.bash_profile` e o do fish, e dotfiles de quem programa costumam ser symlink de um repositório. Quando a pasta está fora do PATH, as Preferências mostram a linha exata para colar. No Windows a conclusão se inverte, e a assimetria é proposital: lá o PATH do usuário não é arquivo pessoal, é o valor `Path` em `HKCU\Environment`, mecanismo por usuário e sem administrador que o rustup e o instalador do Node usam.

Alternativa rejeitada: chamar o executável direto no macOS. Sem `tauri-plugin-single-instance`, isso cria uma segunda instância disputando `preferences.json`, o jornal de terminais, a porta do túnel e o perfil do Dev Browser, e o processo nasce no grupo do shell, morrendo quando o terminal fecha. Também foi rejeitado um segundo `externalBin`, que mexeria em campos que `tools/check/desktop-scaffold.mjs` fixa.
