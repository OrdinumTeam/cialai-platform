...
| --- | --- | --- |
| `ci.yml` | Preparado | Matriz e comandos estão versionados; nenhuma execução remota desta linha foi observada |
...
| --- | --- | --- |
| `ci.yml` | push, PR e manual | Matriz `ubuntu-22.04`, `windows-2022` e `macos-14`; instala Node, npm, Rust e Go nas versões fixadas; executa `npm ci` e `npm test`. Não produz instalador nem substitui os testes físicos |
