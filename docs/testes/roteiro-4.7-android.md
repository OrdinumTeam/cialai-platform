...
| Pessoa responsável |  |
| Fabricante e modelo |  |
| Versão e nível da API do Android |  |
| Versão e build do Cialai móvel |  |
| Origem do AAB ou APK |  |
...
- [ ] Build interno instalado no aparelho real
...
- [ ] Opções de bateria e segundo plano registradas
...
Critério geral: os treze cenários de rede e os quatro cenários Android precisam ser aprovados. Uma falha, um campo obrigatório vazio ou uma medição sem evidência mantém a tarefa 4.7 pendente.
...
Passos: revogue o aparelho enquanto ele envia entrada. Depois faça um novo pareamento.
...
Passos: force a parada do aplicativo e abra novamente.
...
Critérios: a sessão religa em até 5 segundos. O processo continua íntegro ou renasce sem estado parcial.

| Campo | Registro |
| --- | --- |
| Duração em segundo plano |  |
| Processo anterior permaneceu |  |
| Tempo até religar |  |
...
Passos: bloqueie UDP na rede Wi-Fi do aparelho e abra a sessão.
...
Passos: adiante o relógio do Android em 30 minutos, faça o pareamento e depois restaure o horário.
...
Passos: pareie outro celular e depois pareie o primeiro aparelho com outro desktop do mesmo usuário.
...
Passos: negue a câmera e confira a alternativa de colar. Negue acesso à rede local se a versão do Android oferecer essa permissão e tente conectar pelo relé.

Critérios: os pedidos explicam a finalidade. Negar a câmera mantém uma alternativa clara. Negar a rede local força somente o relé quando a permissão existir.

| Campo | Registro |
| --- | --- |
| Mensagem ao negar câmera |  |
| Alternativa de colar funciona |  |
| Permissão de rede local disponível |  |
| Conexão pelo relé |  |
| Evidência |  |
| Resultado | ☐ Aprovado ☐ Reprovado ☐ Bloqueado |
| Observações |  |
...
## 15. Parada após dois minutos

Passos: deixe o app em segundo plano por mais de 120 segundos, registre o processo e retorne.

Critérios: o núcleo recebe `Stop` uma vez. Ao voltar, executa `StartProfile` e `OpenDesktop`, mostra Reconectando e abre a sessão somente depois da saúde verde. O retorno ocorre em até 5 segundos numa rede estável.

| Campo | Registro |
| --- | --- |
| Tempo em segundo plano |  |
| Chamadas `Stop` |  |
| `StartProfile` observado |  |
| `OpenDesktop` observado |  |
| Tempo até saúde verde |  |
| Evidência |  |
| Resultado | ☐ Aprovado ☐ Reprovado ☐ Bloqueado |
| Observações |  |
...
Passos: teste a biometria disponível, o cancelamento e o fallback seguro do aparelho. Repita após cinco minutos fora e antes de uma ação sensível.

Critérios: a sessão pede autenticação depois do prazo. Cada ação sensível exige autorização nova. Cancelar não envia entrada nem abre ou encerra uma sessão.
...
| Ação bloqueada ao cancelar |  |
| Evidência |  |
| Resultado | ☐ Aprovado ☐ Reprovado ☐ Bloqueado |
| Observações |  |

## 17. Armazenamento protegido

Passos: pareie dois desktops, reinicie o aparelho, esqueça um perfil e confira o estado seguro com ferramentas autorizadas de desenvolvimento.

Critérios: cada token permanece separado por desktop. Reiniciar preserva somente os dados esperados. Esquecer remove os tokens do perfil. Backup e armazenamento externo não recebem o estado do túnel.

| Campo | Registro |
| --- | --- |
| Tokens separados |  |
| Reinício preservou o esperado |  |
| Perfil esquecido foi removido |  |
| Backup sem estado do túnel |  |
| Armazenamento externo limpo |  |
| Evidência |  |
| Resultado | ☐ Aprovado ☐ Reprovado ☐ Bloqueado |
| Observações |  |
...
Decisão da tarefa 4.7: ☐ Aprovada ☐ Pendente

Somente marque Aprovada quando os dezessete cenários tiverem resultado aprovado e evidência correspondente.
