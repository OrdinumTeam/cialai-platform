...
| `linux-packages.yml` | só manual, com o commit | Um único job `ubuntu-22.04` refaz o `.deb` e o `.rpm` pelo mesmo caminho do job Linux do `release.yml`, sem AppImage e sem tocar na release: testa `withLibraryDir`, compila o sidecar Linux, prepara o Tor, assina os pacotes para o atualizador, confere as assinaturas, confere que o sidecar empacotado define `LD_LIBRARY_PATH` para o Tor e roda o `tor --version` do pacote. Os arquivos saem como artefato do run. Criado em 15/09/2026 porque o `.deb` e o `.rpm` da 0.2.0 e da 0.2.1 abriam o Tor sem as bibliotecas dele, com saída 127 |
